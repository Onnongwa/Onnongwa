# 🌱 Onnongwa : 체험 등록을 어려워하는 농가를 위해 AI를 활용해 리브랜딩, 맞춤형 체험 추천, 홍보 영상 제작을 제공하는 웹 서비스
<div>
  <img width="1038" height="524" alt="image" src="https://github.com/user-attachments/assets/73cb543b-286b-4603-aab9-915f54d5e907" />
</div>

<br>

# 💡 추진 배경
최근 국내에서는 '저속노화(Slow Aging)' 트렌드가 확산되며 일상에서 벗어나 자연에서 힐링하는 촌캉스, 웰니스 여행이 다시 주목받고 있습니다. 특히 이러한 여행 코스로는 단순한 관광이 아닌 자연과 교감하는 농어촌 체험 형태가 주요 선택지로 떠오르고 있습니다.
<br>

농어촌 체험 프로그램을 검색하고 예약할 수 있는 웹페이지는 일부 존재하지만 그 대부분은 젊은 농어촌 운영자들에 의해 제한적으로 운영되는 경우가 많습니다. 실제 농어촌의 더 큰 비중을 차지하는 중장년층 이상 주민들은 디지털 기술에 익숙하지 않아 자신이 가진 자원이나 프로그램을 어떻게 온라인에 노출하고 사람들을 유입시킬 수 있는지조차 어려움을 겪고 있는 실정입니다. 이로 인해 도시민 입장에서는 다양한 체험 정보를 접하기 어렵고, 농어촌민 입장에서는 본인의 콘텐츠를 알릴 기회조차 갖기 힘든 양방향 단절 현상이 발생하고 있습니다.
<br> 

저희 팀은 이러한 문제의식을 바탕으로 모든 연령대의 농어촌민이 최소한의 정보만 입력하면 AI 기술이 자동으로 소개글을 작성하고 홍보를 매칭해주는 웹 플랫폼을 기획하게 되었습니다. 도시민에게는 더 다양한 체험 기회를 제공하고 예약할 수 있는 O2O(Oline to Offline) 기반 구조를 통해 정보 접근성을 높이며, 농어촌민에게는 복잡한 기술 학습 없이도 손쉽게 자신의 삶과 자원을 공유하고 수익을 창출할 수 있는 기회를 제공합니다. 이를 통해 디지털 격차 해소와 도농 간 상생 구조 구축이라는 사회적 가치를 실현하는 것이 저희 프로젝트의 핵심 목표입니다.

<br>


# 🌿 프로젝트 소개

- 트렌드 데이터 분석과 사용자 선호도 기반 알고리즘을 통해, 이용자에게 맞춤형 농촌 체험 프로그램을 추천
- 농촌 이미지를 현대적으로 재해석하고, 직관적인 UI·UX 디자인으로 사용자가 쉽게 탐색할 수 있는 플랫폼
- 농장주가 직접 체험 프로그램을 등록할 수 있으며, 등록한 체험 프로그램을 기반으로 영상을 제작해 유튜브에 업로드함으로써 홍보 효과를 극대화

<br>

# ⚙️ 주요 기능
  ## 1. 사용자가 입력한 작물, 지역 등을 기반으로 관련 트렌드를 수집하고, OpenAI API를 통해 맞춤형 농촌 체험 프로그램 3가지를 추천
  <div align="center">
    <img width="400" height="463" alt="image" src="https://github.com/user-attachments/assets/bba03246-58b0-435b-bec8-46e49b4442b2" />
    <img width="400" height="463" alt="image" src="https://github.com/user-attachments/assets/d486384a-4373-47e7-b94a-d9e8ce510c9f" />
  </div>
  <div align="center">
  <img width="600" height="550" alt="image" src="https://github.com/user-attachments/assets/7553b537-6f3b-4885-838f-7acdbaec0157" />
  </div>

<br>

 ## 2. OpenAI API를 활용해 기존 체험 정보를 마케팅 관점에서 자연스럽고 매력적인 문체로 재작성하고, 프로그램 일정·하이라이트·해시태그 등을 자동 생성
<div align="center">
  <img width="574" height="599" alt="화면 캡처 2025-11-10 123929" src="https://github.com/user-attachments/assets/917066d9-e9ef-416b-90e2-12a1b748378d" />
  <img width="576" height="617" alt="화면 캡처 2025-11-10 123950" src="https://github.com/user-attachments/assets/00972dc9-9ea4-45c4-a87c-eafcd85aa51c" />
  <img width="559" height="621" alt="화면 캡처 2025-11-10 124050" src="https://github.com/user-attachments/assets/7b8568a6-73ec-4a76-8fc9-9f14b06f4614" />
</div>





<br>

 ## 3. 홍보 영상 제작 및 유튜브 업로드


<br>

# 👥 팀원 역할분담
| <img src="https://avatars.githubusercontent.com/u/107541923?v=4" width="120px;" alt=""/> | <img src="https://avatars.githubusercontent.com/u/84631435?v=4" width="120px;" alt=""/> | <img src="https://avatars.githubusercontent.com/u/128787964?v=4" width="120px;" alt=""/> |
|:----------------------------------------------------------------------------------------:|:---------------------------------------------------------------------------------------:|:----------------------------------------------------------------------------------------:|
|                           [이원희](https://github.com/Leewon2)                           |                           [김재혁](https://github.com/Combro-Kim)                            |                           [전승현](https://github.com/jjsh0208)                           |                           
|                                            체험 추천                                            |                                         홍보 영상 제작 및 유튜브 업로드                                         |                                       체험 리브랜딩                              |

<br>

# 🛠️ 기술 스택
**주요 언어**: **주요 언어**: Java 17, Python, JavaScript/TypeScript(React)
<br>
**빌드 도구**: Gradle 8.10.2  
<table>
  <tr>
    <td><strong>Frontend</strong></td>
    <td>
      React 18<br>
      React Router v6<br>
      React Testing Library
    </td>
    <td align="center">
      <img src="https://upload.wikimedia.org/wikipedia/commons/a/a7/React-icon.svg" width="50"/>
    </td>
  </tr>
  <tr>
    <td><strong>Backend (Java)</strong></td>
    <td>
      Spring Boot 3.5.4<br>
      Spring Data JPA<br>
      Spring Data Redis<br>
      Spring Web / WebFlux<br>
      RestClient & WebClient (OpenAI·Python 연동)<br>
      Lombok, JUnit
    </td>
    <td align="center">
      <img src="https://spring.io/img/projects/spring-boot.svg" width="50"/>
      <img src="https://github.com/user-attachments/assets/2a90d034-1542-47e0-a524-b4febc5f170e" width="50"/>
    </td>
  </tr>
  <tr>
    <td><strong>Database / Caching</strong></td>
    <td>
      MySQL
    </td>
    <td align="center">
      <img src="https://www.mysql.com/common/logos/logo-mysql-170x115.png" width="50"/>
    </td>
  </tr>
  <tr>
    <td><strong>AI / Python</strong></td>
    <td>
      FastAPI <br>
      Uvicorn (서버 런타임)<br>
      OpenAI API (프로그램 추천·리브랜딩 응답 생성)
    </td>
    <td align="center">
      <img src="https://fastapi.tiangolo.com/img/logo-margin/logo-teal.png" width="60"/>
      <img src="https://github.com/user-attachments/assets/56d25a4e-06ea-4a5a-911f-397cd9002593"  width="60"/>
    </td>
  </tr>
</table>


<br>

# ERD
<img  width="675" height="452" alt="image" src="https://github.com/user-attachments/assets/b1799f1c-ee5d-49a1-80f3-14e5a0ef09be" />

<br>

# ☁️ 인프라 아키텍처






<br>

# 🧭 기술적 의사결정

<br>

# 🔧 트러블슈팅

## 1️⃣ [데이터 수집 과정에서 Selenium 크롤링 이슈 해결](https://github.com/Onnongwa/Onnongwa/wiki/%5B%ED%8A%B8%EB%9F%AC%EB%B8%94%EC%8A%88%ED%8C%85%5D%5BPython%5D-%EB%8D%B0%EC%9D%B4%ED%84%B0-%EC%88%98%EC%A7%91-%EA%B3%BC%EC%A0%95%EC%97%90%EC%84%9C-Selenium-%ED%81%AC%EB%A1%A4%EB%A7%81-%EC%9D%B4%EC%8A%88-%ED%95%B4%EA%B2%B0)

## 2️⃣ [_class 필드로 인한 JSON 역직렬화 예외 해결](https://github.com/Onnongwa/Onnongwa/wiki/%5B%ED%8A%B8%EB%9F%AC%EB%B8%94%EC%8A%88%ED%8C%85%5D%5BElasticsearch%5D-_class-%ED%95%84%EB%93%9C%EB%A1%9C-%EC%9D%B8%ED%95%9C-JSON-%EC%97%AD%EC%A7%81%EB%A0%AC%ED%99%94-%EC%98%88%EC%99%B8-%ED%95%B4%EA%B2%B0)

